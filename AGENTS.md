...
- **Sentry:** Replay OFF, tracing OFF: rates 0, no `replayIntegration`; `instrumentation-client.ts` runtime-strips `browserTracingIntegration` (tree-shake no-ops on Turbopack). `sendDefaultPii: false`; `lib/sentry-scrub.ts` `scrubEvent` as `beforeSend` at every init site; EU ingest only. Never `@sentry/wizard`. ⚠ **`request.url` is reduced to origin + pathname — the query string is NOT a separate field on the browser SDK path (T-041, 2026-08-30).** `request.query_string` was deleted for months and is never emitted there; the query rides inside `request.url`, carrying `?visit=<consultation uuid>`, `?q=<free-text admin search>` and `?session=<sessionId>`, the phone-upload credential. **The scrub is a CONTRACT, not a comment:** `public/sentry-scrub-contract.json` is the canonical spec, byte-mirrored into tubermed-backend (`lib/sentry-scrub-contract.json`) and verified there by `scripts/verify-mirror.js`, tokenlessly, on every backend push — so a change here that does not land there goes RED. `lib/sentry-scrub.ts` and the backend's `lib/sentry-scrub.js` are two INDEPENDENT implementations of it and must not read it; each repo executes its own against its own copy (`scripts/sentry-scrub.test.ts` here). Changing what is scrubbed means editing the contract in BOTH repos plus both implementations. The old header claimed to mirror the backend „EXACTLY" while omitting `extra` and `contexts`, which the backend had dropped since B-1 — that is why the claim is now checked.
